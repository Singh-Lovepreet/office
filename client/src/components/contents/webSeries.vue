...
<div>
<span style="font-weight: bold;font-size:14px;">Tags(<a @click="setTags('clear')">clear tags:</a>)</span>
<md-chip  @click="setTags('familySafe')" v-bind:class="{'md-accent':tagsfilter.includes('familySafe')}" md-clickable>family safe</md-chip>
<md-chip  @click="setTags('18+')" v-bind:class="{'md-accent':tagsfilter.includes('18+')}" md-clickable>18+</md-chip>
</div>
<div style="display: flex;justify-content: space-between;flex-direction: row;align-items: center;">
<a @click="languagefilter=''">clear</a>
...
<div class="filteredData" v-if="generefilter.length">
<div style="display: flex; flex-direction: row;align-items: center;text-align: center; justify-content: space-between;" id="headings">
<p class="heading">Filtered Result:({{filteredData.length}} results found)</p>
</div>

<div class="grid-container">
<md-card class="md-card"  v-for="post in filteredData"  v-bind:data="post" :key="post.id">
...
<div style="display: flex;flex-direction: row;height:20px;margin-bottom:10px">
<p style="margin-right: 5px;line-height:30px">Select Yawns:</p>
...
<div v-else>
<div style="display: flex; flex-direction: row;align-items: center;text-align: center; justify-content: space-between;" id="headings">
<p class="heading">All Web Series:</p>
</div>
<div class="grid-container">
...
<md-list>
<md-subheader>Filters(<a @click="setGeneres('clear')">clear all</a>)</md-subheader>
<md-chip  @click="setGeneres(genere)"  v-model="generefilter" class="list-content" 
v-for="genere in generes" v-bind:class="{'md-accent':generefilter.includes(genere.category)}" :key="genere.s_no" md-clickable>
<span>{{genere.category}}</span>
</md-chip>
</md-list>
</div>
</div>
...
// if(this.tagsfilter.length || tag==='clear'){
...
// },
setGeneres(genere){
if(this.generefilter.length || genere==='clear'){

	this.generefilter.length=0;
}
if(genere!=='clear'){

this.generefilter.push(genere.category);
this.filterWebSeries();

}

else{

this.generefilter.pop();	

}

},

async filterWebSeries(){

const response = await PostsService.fetchFilteredSeries({

query:this.generefilter[0]
})
if(response){
this.loading=false
this.filteredData= response.data.series
}
...
async getPosts () {
const response = await PostsService.fetchWebSeries()
if(response){
this.loading=false
this.Series= response.data.series
// this.languages=JSON.parse(response.data.languages)
this.generes=JSON.parse(response.data.generes);
}      
},
authenticate(){
