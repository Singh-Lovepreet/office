...
<router-link to="/">
<img alt=""  src=".././assets/logo.png" height="70" width="70" style="margin-left:-15px">
</router-link>
...
<md-card class="md-card"  v-for="post in singlePost"  v-bind:data="post" :key="post._id">
...
<md-chip v-if="post.tags" md-disabled id="chip" v-for="tag in post.tags" :key="post.id">       
...
const response = await PostsService.getMappedPost({
tokenId:token,
id: this.$route.params.id
})
if(response){
this.loading=false
this.singlePost=response.data.Episodes
}
},
...
async seriesClicked (post) {
const token=this.$store.getters.authHeader;
await PostsService.seriesCount({
tokenId:token||'',
id:post._id
})
},
